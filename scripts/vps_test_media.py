    ...
    async with async_session_factory() as db:
        ...
        for d in dialogs:
            if "cityboy" in (d.name or "").lower():
                print(f"Found dialog: {d.name}, id={d.id}")
                msgs = await client.get_messages(d.entity, limit=10)
                for m in msgs:
                    if m.media:
                        print(f"Message {m.id} has media: {m.media.__class__.__name__}")
                        # Test download
                        import io
                        buf = io.BytesIO()
                        res = await client.download_media(m, file=buf)
                        print(f"Downloaded bytes: {len(buf.getvalue())}")
                        break
                break
