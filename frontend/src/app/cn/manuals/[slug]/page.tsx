...
const seoCards = [
  ...
  { title: "客户评价", desc: "真实用户的评价和意见", href: "/cn/reviews" },
  { title: "优势", desc: "为什么选择我们？简要列出我们的优势", href: "/cn/telegram-promotion" },
  { title: "合作伙伴", desc: "为代理商和大客户提供的优惠条件", href: "/cn/refferal" },
];

export default async function ManualPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const content = manualContent[slug];
  if (!content) redirect("/cn/manuals");
        ...
        <section className="pt-28 pb-8 lg:pt-36 lg:pb-10 relative">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="flex items-start gap-6">
              ...
              <div>
                <Link href="/cn/manuals" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors mb-2">
