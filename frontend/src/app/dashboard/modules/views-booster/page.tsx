...
import { Eye, ArrowLeft, Play, Loader2, BarChart3, Link, Users, Globe } from "lucide-react";
...
export default function ViewsBoosterPage() {
  const router = useRouter();
  const [accounts, setAccounts] = useState<any[]>([]);
  const [selectedAccounts, setSelectedAccounts] = useState<string[]>([]);
  const [postUrl, setPostUrl] = useState("");
  const [viewCount, setViewCount] = useState(100);
  const [mode, setMode] = useState<"accounts" | "proxy" | "direct">("accounts");
  ...
  async function handleExecute() {
    ...
    try {
      addLog(`Boosting views for ${postUrl}...`);
      const op = mode === "accounts" ? "boost_direct_views" : mode === "proxy" ? "boost_proxy_views" : "boost_post_views";
      const params: any = { post_url: postUrl, count: viewCount, thread_count: threadCount, delay_before_action: `${minDelay}-${maxDelay}` };
      if (mode === "accounts") params.account_phones = selectedAccounts;
      if (mode === "proxy") params.proxy_list = proxyList.split("\n").map(s => s.trim()).filter(Boolean);

      const r = await api.post("/modules/views_boost/execute", { operation: op, params });
      const res = r.data?.result || r.data;
      setResults(res);
      addLog(`Boosted ${res.views || res.count || "?"} views`, "success");
    } catch (e: any) {
    ...
    <div className="min-h-screen pb-20">
      <div className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">Views Booster</h1>
            <p className="text-xs text-muted-foreground">Increase post and channel views</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-3">
            {[
              { id: "direct", label: "Direct", icon: Link },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">View Count</label>
              <input type="number" min={10} max={100000} value={viewCount} onChange={e => setViewCount(parseInt(e.target.value) || 100)}
                className="w-full bg-secondary border-0 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary" />
            </div>
          </div>
          ...
          <button onClick={handleExecute} disabled={executing || !postUrl}
            ...
            {executing ? "Boosting..." : "Boost Views"}
          </button>
        </div>
        ...
        {results && (
          <div className="bg-card rounded-xl border border-border p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-muted-foreground">Views:</span> <span className="text-foreground font-medium">{results.views || results.count || "—"}</span></div>
              <div><span className="text-muted-foreground">Status:</span> <span className="text-green-500">{results.status || "completed"}</span></div>
            </div>
          </div>
        )}
