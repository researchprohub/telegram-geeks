...
import { UserPlus, ArrowLeft, Play, Loader2, Users, Phone, Hash } from "lucide-react";
...
export default function InviteV1Page() {
  const router = useRouter();
  const [accounts, setAccounts] = useState<any[]>([]);
  const [accountId, setAccountId] = useState("");
  const [targetGroup, setTargetGroup] = useState("");
  const [phoneNumbers, setPhoneNumbers] = useState("");
  const [mode, setMode] = useState<"numbers" | "members">("numbers");
  ...
  async function handleExecute() {
    if (!accountId || !targetGroup) { setError("Select account and enter group"); return; }
    if (mode === "numbers" && !phoneNumbers.trim()) { setError("Enter phone numbers"); return; }
    setExecuting(true); setError("");
    try {
      addLog(`Inviting users to ${targetGroup}...`);
      const r = await api.post("/modules/invite_modules/execute", {
        operation: mode === "numbers" ? "invite_by_numbers" : "invite_by_members",
        params: {
          account_id: accountId,
          chat_id: targetGroup,
          phone_numbers: mode === "numbers" ? phoneNumbers.split("\n").map(s => s.trim()).filter(Boolean) : undefined,
    ...
    <div className="min-h-screen pb-20">
      <div className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">Invite V1</h1>
            <p className="text-xs text-muted-foreground">Invite users by phone numbers or member lists</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-3">
            {[
              { id: "numbers", label: "By Numbers", icon: Phone },
              { id: "members", label: "By Members", icon: Users },
            ].map(m => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={targetGroup} onChange={e => setTargetGroup(e.target.value)} placeholder="https://t.me/group or @username"
                className="w-full bg-secondary border-0 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary" />
            </div>
          </div>
          ...
          <button onClick={handleExecute} disabled={executing || !accountId || !targetGroup}
            ...
            {executing ? "Inviting..." : "Start Invite"}
          </button>
        </div>
        ...
        <CrossLinkFooter links={[
          { label: "Invite V2", href: "/dashboard/modules/invite-v2" },
          { label: "Invite by ID", href: "/dashboard/modules/invite-by-id" },
          { label: "Invite via Admin V1", href: "/dashboard/modules/invite-via-admin-v1" },
        ]} />
