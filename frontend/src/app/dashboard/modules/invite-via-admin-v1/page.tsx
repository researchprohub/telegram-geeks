...
import { Shield, ArrowLeft, Play, Loader2 } from "lucide-react";
...
export default function InviteViaAdminV1Page() {
  ...
  async function handleExecute() {
    ...
    try {
      ...
      addLog(`Inviting ${ids.length} users via admin to ${targetGroup}...`);
      const r = await api.post("/modules/invite_modules/execute", {
        operation: "invite_via_admin",
      ...
      addLog(`Invited ${res.invited || "?"} users via admin`, "success");
    ...
    <div className="min-h-screen pb-20">
      <div className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-lg font-bold text-foreground">Invite via Admin V1</h1><p className="text-xs text-muted-foreground">Invite users as group admin (bypass join requests)</p></div>
        </div>
      </div>
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          ...
          <div className="mt-3">
            <label className="block text-xs text-muted-foreground mb-1">User IDs to Invite</label>
          ...
          <button onClick={handleExecute} disabled={executing || !accountId || !targetGroup || !userIds.trim()}
            ...
            {executing ? "Inviting..." : "Invite as Admin"}
          </button>
        </div>
