...
import { UserPlus, ArrowLeft, Play, Loader2, Zap, Users } from "lucide-react";
...
export default function InviteV2Page() {
  ...
  async function handleExecute() {
    ...
    try {
      addLog(`Smart inviting from ${sourceGroup} → ${targetGroup}...`);
      const r = await api.post("/modules/invite_modules/execute", {
        operation: "smart_invite",
        params: {
          account_id: accountId,
          source_chat_id: sourceGroup,
          target_chat_id: targetGroup,
      ...
      addLog(`Invited ${res.invited || "?"}/${maxInvites} users`, "success");
    ...
    <div className="min-h-screen pb-20">
      <div className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">Invite V2 (Smart)</h1>
            <p className="text-xs text-muted-foreground">Smart invite: collect then invite</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" min={1} max={500} value={maxInvites} onChange={e => setMaxInvites(parseInt(e.target.value) || 50)}
                className="w-full bg-secondary border-0 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Source Chat (collect from)</label>
              <input type="text" value={sourceGroup} onChange={e => setSourceGroup(e.target.value)} placeholder="https://t.me/source"
                className="w-full bg-secondary border-0 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Target Chat (invite to)</label>
              <input type="text" value={targetGroup} onChange={e => setTargetGroup(e.target.value)} placeholder="https://t.me/target"
                className="w-full bg-secondary border-0 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary" />
            </div>
          </div>
          <button onClick={handleExecute} disabled={executing || !accountId || !sourceGroup || !targetGroup}
            ...
            {executing ? "Inviting..." : "Start Smart Invite"}
          </button>
        </div>
