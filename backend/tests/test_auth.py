...
# Test /me endpoint
req2 = urllib.request.Request("http://localhost:8000/api/v1/auth/me", headers={"Authorization": f"Bearer {access_token}"})
resp2 = urllib.request.urlopen(req2)
me_data = json.loads(resp2.read())
print(f"/me response: {me_data}")

# Test /accounts endpoint
req3 = urllib.request.Request("http://localhost:8000/api/v1/accounts/", headers={"Authorization": f"Bearer {access_token}"})
